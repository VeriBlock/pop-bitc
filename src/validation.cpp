...
public:
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool disnicknames);
...
private:
    ...
    // VeriBlock
    bool TestBlockIndex(CBlockIndex* inhdex);
} g_chainstate;
...
bool CheckSequenceLocks(const CTransaction &tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    if (useExistingLockPoints) {
        ...
        for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
            ...
            Coin coin;
            if (!txin.isnickname && !viewMemPool.GetCoin(txin.prevout, coin)) {
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
    ...
    if (fRequireStandard && !IsStandardTx(tx, reason, witnessEnabled)){
        LogPrintf("IsStandardTx rejected the transaction\n");
                ...
                for (const CTxIn &mtxin : mtx.vin)
                {
                    if (mtxin.isnickname){                        
                        ...
                        if (txin.address==mtxin.address && !nicknamemasterpubkey.Verify(hash, txin.nicknamesig)) {
                            LogPrintf("There is already a transaction with this address for the nickname in the mempool.\n");
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                if (fReplacementOptOut) {
                    LogPrintf("txn-mempool-conflict\n");
...

    {
        ...
        if (fRequireStandard && !AreInputsStandard(tx, view)){
            LogPrintf("bad-txns-nonstandard-inputs\n");
            return state.Invalid(false, REJECT_NONSTANDARD, "bad-txns-nonstandard-inputs");
        }

        // Check for non-standard witness in P2WSH
        if (tx.HasWitness() && fRequireStandard && !IsWitnessStandard(tx, view)){
            LogPrintf("bad-witness-nonstandard\n");
        ...
        if (nSigOpsCost > MAX_STANDARD_TX_SIGOPS_COST) {
            LogPrintf("bad-txns-too-many-sigops\n");
        ...
        if (!bypass_limits && mempoolRejectFee > 0 && nModifiedFees < mempoolRejectFee) {
            LogPrintf("mempool min fee not met\n");
        ...
        for (CTxMemPool::txiter ancestorIt : setAncestors)
        {
            ...
            {
                LogPrintf("bad-txns-spends-conflicting-tx\n");
        ...
        if (fReplacementTransaction)
        {
            ...
            if (nConflictingCount <= maxDescendantsToVisit) {
                ...
                LogPrintf("too many potential replacements\n");
                return state.DoS(0, false,
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++)
            {
                ...
                if (!setConflictsParents.count(tx.vin[j].prevout.hash))
                {
                    ...
                    if (pool.mapTx.find(tx.vin[j].prevout.hash) != pool.mapTx.end()) {
                        LogPrintf("replacement-adds-unconfirmed\n");
                        return state.DoS(0, false,
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata) && !isminttransaction) {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                LogPrintf("BUG! PLEASE REPORT THIS! ConnectInputs failed against latest-block but not STANDARD flags\n");
                return error("%s: BUG! PLEASE REPORT THIS! ConnectInputs failed against latest-block but not STANDARD flags %s, %s",
                    __func__, hash.ToString(), FormatStateMessage(state));
            } else {
                if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, txdata)) {
                    LogPrintf("ConnectInputs failed against MANDATORY but not STANDARD flags due to promiscuous mempool\n");
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BITC additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool disnicknames)
{
    bool fClean = true;
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                if (!is_spent || tx.vout[o].nValue != coin.out.nValue || tx.vout[o].scriptPubKey != coin.out.scriptPubKey || pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase) {

                    if (is_coinbase != coin.fCoinBase)
                    error("is_coinbase != coin.fCoinBase");
                    error("Isspent: %b Height: %d Height2: %d", is_spent,pindex->nHeight,coin.nHeight);
            ...
            if (!paysdev1) 
            {
                return state.DoS(100,
                             error("ConnectBlock(): coinbase does not pay developers"),
                                   REJECT_INVALID, "bad-cb-dev-payment");
            }
        }
...
CBlockIndex* CChainState::FindBestChain() {
    AssertLockHeld(cs_main);
    CBlockIndex* bestCandidate = chainActive.Tip();
...
bool CChainState::TestBlockIndex(CBlockIndex* pindexTest)
{
    ...
    while (testWalkBlock && !chainActive.Contains(testWalkBlock)) {
        assert(testWalkBlock->nChainTx || testWalkBlock->nHeight == 0);
        ...
        if (fFailedChain || fMissingData) {
            ...
            while (testWalkBlock != pindexFailed) {
                if (fFailedChain) {
                    std::cout << "failed block1 " << pindexFailed->nHeight << ", status " << pindexFailed->nStatus << "\r\n";
        ...
        bool fInitialDownload;
        {
            ...
            if (pblock && pindexBestChain == nullptr) {
                ...
                for (auto* candidate : tmp_set) {
                    ...
                    if ((candidate->nChainTx != 0) && TestBlockIndex(candidate) && candidate->GetAncestor(blockindex->nHeight) == blockindex) {
            ...
            if (pindexBestChain == nullptr || pindexBestChain == chainActive.Tip())
                return true;
...
bool CChainState::ReceivedBlockTransactions(const CBlock &block, CValidationState& state, CBlockIndex *pindexNew, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    if (pindexNew->pprev == nullptr || pindexNew->pprev->nChainTx) {
        ...
        while (!queue.empty()) {
            ...
            // VeriBlock: always insert candidates
        ...
        for (unsigned int j = 0; j < tx.vin.size(); j++) 
        {
            if (tx.vin[j].isnickname) 
            {
                ...
                if (std::find(addrs.begin(), addrs.end(),tx.vin[j].address)!=addrs.end()){
                    ...
                    return state.DoS(100, false, REJECT_INVALID, "bad-nickname-address", false, "more than one identical address for nickname");
                }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool fCheckMerkleRoot)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            return state.DoS(100, false, REJECT_INVALID, "bad-cb-height", false, strprintf("block height mismatch in coinbase for block %d", nHeight));
        }
    }
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if (!fRequested) {  // If we didn't ask for it:
        ...
        // VeriBlock: we do not have cumulative block work
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), false) ||
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        if (pindex->pprev)
            pindex->BuildSkip();
        // VeriBlock: do not set best chain here
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.DynamicMemoryUsage() + pcoinsTip->DynamicMemoryUsage()) <= nCoinCacheUsage) {
            ...
            DisconnectResult res = g_chainstate.DisconnectBlock(block, pindex, coins, false);
...
bool CChainState::ReplayBlocks(const CChainParams& params, CCoinsView* view)
{
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            DisconnectResult res = DisconnectBlock(block, pindexOld, cache, true);
            if (res == DISCONNECT_FAILED) {
