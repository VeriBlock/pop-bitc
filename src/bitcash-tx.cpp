...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = strprintf(_("%s bitcash-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  bitcash-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded bitcash transaction") + "\n" +
              "  bitcash-tx [options] -create [commands]   " + _("Create hex-encoded bitcash transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CTxDestination destination = DecodeDestinationNoNickname(strAddr);
    ...
    CTxOut txout(value, scriptPubKey, 0);
...
static void MutateTxAddOutData(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CTxOut txout(value, CScript() << OP_RETURN << data, 0);
    tx.vout.push_back(txout);
}
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcash transaction
