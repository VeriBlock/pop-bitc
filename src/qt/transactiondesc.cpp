...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit,WalletModel *model)
{
    int numBlocks;
    ...
    CAmount nNetbitcoin = nCreditbitcoin - nDebitbitcoin;
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                if (!wtx.value_map.count("to") || wtx.value_map["to"].empty())
                {
                    ...
                    if (found)
                    {
                        ...
                        if (wallet.getAddress(
                            ...
                            strHTML += GUIUtil::HtmlEscape(name) + " ";
                        strHTML += GUIUtil::HtmlEscape(EncodeDestinationHasSecondKey(address));
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + BitcashUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
    ...
    strHTML += "<b>" + tr("Net amount BTC") + ":</b> " + BitcashUnits::formatHtmlWithUnit(BitcashUnits::BITCOIN, nNetbitcoin, true) + "<br>";
    ...
    // Message from normal bitcash:URI (bitcash:123...?message=example)
