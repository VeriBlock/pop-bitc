...
public:
    ...
    QString describe(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit,  WalletModel *model)
    {
        return TransactionDesc::toHTML(node, wallet, rec, unit, model);
    }
        ...
        platformStyle(_platformStyle)
{
    ...
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << tr("Reference line") << BitcashUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
int TransactionTableModel::TxTypeAsNumber(const TransactionRecord *wtx) const
{
    switch(wtx->type)
    {
    case TransactionRecord::RecvWithAddress:
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, BitcashUnits::SeparatorStyle separators) const
{
    QString str = BitcashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false, separators);
    if(showUnconfirmed)
    {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case TableTypeRole:
            return formatTxType(rec);
    case TableTypeAsNumberRole:
            return TxTypeAsNumber(rec);
    case TableAddressRole:
            return formatTxToAddress(rec, false);
    ...
    case LongDescriptionRole:
        return priv->describe(walletModel->node(), walletModel->wallet(), rec, walletModel->getOptionsModel()->getDisplayUnit(),walletModel);
    case AddressRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, BitcashUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
...
QHash<int, QByteArray> TransactionTableModel::roleNames() const {
    ...
    roles[TableDateRole] = "transactiondate";
