...
bool CTxMemPoolEntry::IsNicknameTx() const
{
    bool isnick=true; 
    for (unsigned int i = 0; i < tx->vin.size(); i++) {    
        if (!tx->vin[i].isnickname){isnick=false;break;}
    }
    return isnick;
}
...
void CTxMemPool::removeConflicts(const CTransaction &tx)
{
    ...
    for (const CTxIn &txin : tx.vin) {
        ...
        if (it != mapNextTx.end()) {
            ...
            {
                bool isnick=true;
                for (const CTxIn &txin2 : txConflict.vin) {
                   if (!txin2.isnickname) isnick=false; 
                }
                if (!isnick) {                
...
static void CheckInputsAndUpdateCoins(const CTransaction& tx, CCoinsViewCache& mempoolDuplicate, const int64_t spendheight)
{
    ...
    CAmount txfee = 0;
    bool fCheckResult = tx.IsCoinBase() || Consensus::CheckTxInputs(tx, state, mempoolDuplicate, spendheight, txfee, uint256S("0x0"), false, NULL);
    assert(fCheckResult);
