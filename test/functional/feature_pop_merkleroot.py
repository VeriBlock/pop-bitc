...
class PoPMerkleRootTest(BitcashTestFramework):
    ...
    def run_test(self):
        ...
        # create a block
        active = False
        for softfork in self.nodes[0].getblockchaininfo()['softforks']:
            if softfork['id'] == 'pop_security':
                active = softfork['reject']['status']

        assert active, "POP is not activated"
